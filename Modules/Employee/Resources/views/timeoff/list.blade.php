                            ...
                            swal({
                                ...
                                function(inputValue){
                                    if(inputValue==name){
                                        $.ajax({
                                            type : "POST",
                                            url : "{{url('employee/timeoff/delete')}}/"+id,
                                            data : {
                                                '_token' : '{{csrf_token()}}'
                                            },
                                            success : function(response) {
                                                if (response.status == 'success') {
                                                    swal("Canceled!", "Employee request time off has been canceled.", "success")
                                                    SweetAlert.init()
                                                    location.href = "{{url('employee/timeoff')}}";
                                                }
...
@section('content')
    ...
    <div class="portlet light bordered">
        <div class="portlet-title">
            <div class="caption">
                <span class="caption-subject font-blue sbold uppercase">List Employee Request Time Off</span>
            </div>
        </div>
                        ...
                        <tbody>
                            @if(!empty($data))
                                @foreach($data as $req_time_off)
                                    <tr data-id="{{ $req_time_off['id_employee_time_off'] }}">
