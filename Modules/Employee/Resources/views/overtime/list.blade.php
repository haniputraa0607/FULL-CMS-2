            ...
            return {
                init: function() {
                    $(".sweetalert-delete").each(function() {
                        ...
                        $(this).click(function() {
                            swal({
                                    title: "Are you sure want to delete this request overtime?",
                                ...
                                function(inputValue){
                                    if(inputValue==name){
                                        $.ajax({
                                            type : "POST",
                                            url : "{{url('employee/overtime/delete')}}/"+id,
                                            data : {
                                                '_token' : '{{csrf_token()}}'
                                            },
                                            success : function(response) {
                                                if (response.status == 'success') {
                                                    swal("Canceled!", "Employee request overtime has been canceled.", "success")
                                                    SweetAlert.init()
                                                    location.href = "{{url('employee/overtime')}}";
                                                }
                                                else if(response.status == "fail"){
                                                    swal("Error!", "Failed to cancel request overtime.")
                                                }
                                                else {
                                                    swal("Error!", "Something went wrong. Failed to cancel equest overtime.")
                                                }
                                            }
                                        });
                                    }else if(inputValue==''){
...
@section('content')
    ...
    <?php
        ...
        if(Session::has('filter-employee-list-overtime')){
            $search_param = Session::get('filter-employee-list-overtime');
    ...
    <form id="form-sorting" action="{{url()->current()}}?filter=1" method="POST">
        @include('employee::overtime.filter')
    </form>
    <br>
    ...
    <div class="portlet light bordered">
        <div class="portlet-title">
            <div class="caption">
                <span class="caption-subject font-blue sbold uppercase">List Employee Request Overtime</span>
            </div>
        </div>
                ...
                <form id="form-sorting" action="{{url()->current()}}?sorting=1" method="POST">
                    ...
                    <div class="col-md-3">
                        <select name="order" class="form-control select2" style="width: 100%">
                            ...
                            <option value="name_employee" @if(isset($order) && $order == 'name_employee') selected @endif>Hair Stylist Name</option>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover text-center" id="kt_datatable">
                        ...
                        <tr>
                            ...
                            @if(MyHelper::hasAccess([512], $grantedFeature))
                        ...
                        <tbody>
                            @if(!empty($data))
                                @foreach($data as $req_time_off)
                                    <tr data-id="{{ $req_time_off['id_employee_overtime'] }}">
                                        <td>{{date('d F Y H:i', strtotime($req_time_off['created_at']))}}</td>
                                        <td>{{$req_time_off['name']}}</td>
                                        ...
                                        <td>
                                            @if(MyHelper::hasAccess([512], $grantedFeature))
                                            <a href="{{ url('employee/overtime/detail/'.$req_time_off['id_employee_overtime']) }}" class="btn btn-sm blue text-nowrap"><i class="fa fa-pencil"></i> Edit</a>
                                            @endif
                                            @if(MyHelper::hasAccess([513], $grantedFeature))
                                            <a class="btn btn-sm red sweetalert-delete btn-primary" data-id="{{ $req_time_off['id_employee_overtime'] }}" data-name="{{ $req_time_off['name'] }}"><i class="fa fa-times"></i> Cancel</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
