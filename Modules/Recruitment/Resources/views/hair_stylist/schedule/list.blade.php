...
@section('content')
    ...
    if(Session::has('filter-hs-schedule')){
        $search_param = Session::get('filter-hs-schedule');
    ...
    <form role="form" class="form-horizontal" action="{{url()->current()}}?filter=1" method="POST">
        {{ csrf_field() }}
        @include('recruitment::hair_stylist.schedule.filter')
    </form>
                    ...
                    <tr>
                        <td>
                            @if(MyHelper::hasAccess([348,349], $grantedFeature))
                                <a class="btn btn-sm btn-info" target="_blank" href="{{ url('recruitment/hair-stylist/schedule/detail', $val['id_hairstylist_schedule']) }}"><i class="fa fa-edit"></i></a>
                            @endif
                        </td>
                        <td>{{ $val['nickname'] }}</td>
                        <td>{{ $val['fullname'] }}</td>
