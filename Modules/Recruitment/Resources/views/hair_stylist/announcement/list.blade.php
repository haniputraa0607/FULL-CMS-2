                            ...
                            swal({
                                ...
                                function(){
                                    $.ajax({
                                        ...
                                        data : "_token="+token+"&id_hairstylist_announcement="+id,
...
@section('content')
    ...
    if(Session::has('filter-hs-announcement')){
        $search_param = Session::get('filter-hs-announcement');
    ...
    <form role="form" class="form-horizontal" action="{{url()->current()}}?filter=1" method="POST">
        {{ csrf_field() }}
        @include('recruitment::hair_stylist.announcement.filter')
    </form>
            ...
            @if(!empty($data))
                @foreach($data as $val)
                    <tr>
                        ...
                        <td word-wrap>
                        	@if(isset($val['hairstylist_announcement_rule_parents']))
								<div class="row static-info">
									<div class="col-md-4 name">Conditions</div>
									<div class="col-md-8 value">: </div>
								</div>
								@php $i=0; @endphp
								@foreach($val['hairstylist_announcement_rule_parents'] as $ruleParent)
									<div class="portlet light bordered" style="margin-bottom:10px">
										@foreach($ruleParent['rules'] as $rule)
											<div class="row static-info">
												...
												@elseif($rule['subject'] == 'id_brand')
													{{ $brands[$rule['parameter']] ?? null }}
												@elseif($rule['subject'] == 'hairstylist_level')
													{{ $rule['parameter'] ?? null }}
												@else
													{{$rule['parameter']}}
												@endif
									...
									@if(count($val['hairstylist_announcement_rule_parents']) > 1 && $i < count($val['hairstylist_announcement_rule_parents']) - 1)
						...
						<td nowrap>
							<a href="{{ url('recruitment/hair-stylist/announcement/edit') }}/{{ $val['id_hairstylist_announcement'] }}" class="btn btn-sm blue"><i class="fa fa-search"></i></a>
							<a class="btn btn-sm red delete" href="{{ url('recruitment/hair-stylist/announcement/delete', $val['id_hairstylist_announcement']) }}" data-toggle="confirmation" data-placement="top"><i class="fa fa-trash-o"></i></a>
						</td>
                    </tr>
                @endforeach
            @else
