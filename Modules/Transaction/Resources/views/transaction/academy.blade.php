    ...
    <script type="text/javascript">
        ...
        $('#table-academy').dataTable({
            ajax: {
                ...
                type: 'GET',
                data: function (data) {
                    const info = $('#table-academy').DataTable().page.info();
            ...
            columns: [
                {
                    ...
                    render: function(value, type, row) {
                        ...
                        const buttons = [
                            `<a class="btn blue btn-sm btn-outline" href="{{url('transaction/academy/detail')}}/${row.id_transaction}">Detail Transaction ${tooltipDetailTransaction}</a>`
                        ];
        ...
        $('#table-academy').on('click', '.confirm-btn', function() {
        ...
        $('#table-academy').on('click', '.retry-btn', function() {
            ...
            $.post("{{url('transaction/retry-void-payment/retry')}}", {
                ...
                $('#table-academy').DataTable().ajax.reload(null, false);
                $.unblockUI();
            });
        });

        $('#table-academy').on('click', '.detail-btn', function() {
            const data = $(this).data('data');
...
@section('content')
    ...
    <div class="portlet light portlet-fit bordered">
        ...
        <div class="portlet-body">
            <table class="table table-striped table-bordered table-hover" width="100%" id="table-academy">
            <thead>
              <tr>
                  <th>Actions</th>
                  <th>Date</th>
