...
@section('page-script')
    ...
    <script type="text/javascript">
        ...
        rules = {
            ...
            status:{
                display:'Hair Stylist Status',
                operator:[],
                opsi:[
                    ['Waiting Complete Payment', 'Waiting Complete Payment'],
                    ['Finding Hair Stylist', 'Finding Hair Stylist'],
                    ['Get Hair Stylist', 'Get Hair Stylist'],
        ...
        $('#table-home_service').dataTable({
            ajax: {
                url : "{{url()->current()}}",
                type: 'GET',
                data: function (data) {
                    const info = $('#table-home_service').DataTable().page.info();
            ...
            columns: [
                {
                    ...
                    render: function(value, type, row) {
                        ...
                        const buttons = [
                        	`<a class="btn blue btn-sm btn-outline" href="{{url('transaction/home-service/manage/detail')}}/${row.id_transaction}">Detail Transaction ${tooltipDetailTransaction}</a>`
                        ];
                ...
                },
                {
                    ...
                    render: function(value, type, row) {
                        if(row.status == null){
                            return '<div class="badge badge-secondary">Waiting Complete Payment</div>';
        ...
        $('#table-home_service').on('click', '.confirm-btn', function() {
        ...
        $('#table-home_service').on('click', '.retry-btn', function() {
            ...
            $.post("{{url('transaction/retry-void-payment/retry')}}", {
                ...
                $('#table-home_service').DataTable().ajax.reload(null, false);
                $.unblockUI();
            });
        });

        $('#table-home_service').on('click', '.detail-btn', function() {
            const data = $(this).data('data');
...
@section('content')
    ...
    <div class="portlet light portlet-fit bordered">
        ...
        <div class="portlet-body">
            <table class="table table-striped table-bordered table-hover" width="100%" id="table-home_service">
            <thead>
              <tr>
                  <th>Actions</th>
                  <th>Date</th>
