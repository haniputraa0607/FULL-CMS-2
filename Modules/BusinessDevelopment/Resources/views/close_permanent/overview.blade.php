            ...
            return {
                init: function() {
                    $(".sweetalert-reject-pengajuan").each(function() {
                        ...
                        var data = {
                                    '_token' : '{{csrf_token()}}',
                                    'id_partners_close_permanent':id
                                        };
                        $(this).click(function() {
                            swal({
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover" id="kt_datatable">
                ...
                <tbody style="text-align:center">
                        @php $i = 1;
                        @endphp
                        @foreach($close_permanent as $value)
                            <tr data-id="{{ $value['id_partners_close_permanent'] }}">
                                <td>{{$i}}</td>
                                <td>{{$value['title']}}</td>
                                ...
                                <td>
                                    ...
                                    @if($value['status'] == 'Process')
                                        <a class="btn btn-sm red sweetalert-reject-pengajuan btn-primary" data-id="{{ $value['id_partners_close_permanent'] }}"><i class="fa fa-trash-o"></i> Reject</a>
                                    @endif
                                </td>
                            </tr> 
                         @php $i++;
