            ...
            return {
                init: function() {
                    $(".sweetalert-document-delete").each(function() {
                        ...
                        var data = {
                                    '_token' : '{{csrf_token()}}',
                                    'id_partners_close_temporary_document':id
                                        };
                        $(this).click(function() {
                            swal({
                                ...
                                function(){
                                    $.ajax({
                                        type : "POST",
                                        url : "{{url('businessdev/partners/close-temporary/lampiran/delete')}}",
                                        data : data,
            ...
            <table class="table table-striped table-bordered table-hover" id="kt_datatable">
                ...
                <tbody style="text-align:center">
                        ...
                        @foreach($lampiran as $value)
                            <tr data-id="{{ $value['id_partners_close_temporary_document'] }}">
                                <td>{{$i}}</td>
                                <td>{{$value['title']}}</td>
                                <td>{{$value['note']}}</td>
                                ...
                                @if($result['status']=="Process")
                                <td>
                                    <a class="btn btn-sm red sweetalert-document-delete btn-primary" data-id="{{ $value['id_partners_close_temporary_document'] }}" data-name="{{$value['title']}}"><i class="fa fa-trash-o"></i> Delete</a>
                                </td>
                                @endif
                            </tr>
                         @php $i++;
