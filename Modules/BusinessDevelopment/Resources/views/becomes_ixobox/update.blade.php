...
<script>
            ...
            return {
                init: function() {
                    $(".sweetalert-reject").each(function() {
                        ...
                        var data = {
                                    '_token' : '{{csrf_token()}}',
                                    'id_partners_becomes_ixobox':{{$result['id_partners_becomes_ixobox']}}
                                        };
                        $(this).click(function() {
                            swal({
                        ...
                        $(this).click(function() {
                            swal({
                                ...
                                function(){
                                    $.ajax({
                                        type : "POST",
                                        url : "{{url('businessdev/partners/becomes-ixobox/success')}}",
                                        data : data,
                                        success : function(response) {
                                            if (response.status == 'success') {
    ...
    <div class="portlet-body form">
                @if(!empty($result['close_date']))
                    <form class="form-horizontal" role="form" action="{{url('businessdev/partners/becomes-ixobox/update')}}" method="post" enctype="multipart/form-data">
                @else
                    <form class="form-horizontal" role="form" action="{{url('businessdev/partners/becomes-ixobox/updateActive')}}" method="post" enctype="multipart/form-data">
                @endif
                        <div class="form-body">
