	...
	<div class="mt-radio-list">
		<label class="mt-radio mt-radio-outline"> Nominal
			<input type="radio" value="Nominal" name="discount_type" 
				@if( (old('discount_type')??$result['deals_discount_delivery_rules']['discount_type']??$result['deals_promotion_discount_delivery_rules']['discount_type']??false) == "Nominal") checked @endif required/>
			<span></span>
		</label>
		<label class="mt-radio mt-radio-outline"> Percent
			<input type="radio" value="Percent" name="discount_type" @if( (old('discount_type')??$result['deals_discount_delivery_rules']['discount_type']??$result['deals_promotion_discount_delivery_rules']['discount_type']??false) == "Percent") checked @endif required/>
			<span></span>
		</label>
	</div>
</div>
<div class="form-group" id="discount-delivery-div" @if( !(old('discount_value')??$result['deals_discount_delivery_rules']??$result['deals_promotion_discount_delivery_rules']??false) ) style="display: none;" @endif >
	<div class="row">
		<div class="col-md-3">
			...
			<div class="input-group @if( (old('discount_type')??$result['deals_discount_delivery_rules']['discount_type']??$result['deals_promotion_discount_delivery_rules']['discount_type']??false) == "Percent") col-md-5 @else col-md-12 @endif" id="discount-delivery-group">
				<div class="input-group-addon" id="delivery-addon-rp" @if( (old('discount_type')??$result['deals_discount_delivery_rules']['discount_type']??$result['deals_promotion_discount_delivery_rules']['discount_type']??false) == "Percent") style="display: none;" @endif>IDR</div>
				<input required type="text" class="form-control text-center" name="discount_value" placeholder="" value="{{ old('discount_value')??$result['deals_discount_delivery_rules']['discount_value']??$result['deals_promotion_discount_delivery_rules']['discount_value']??null }}" min="0" oninput="validity.valid||(value='');" autocomplete="off">
				<div class="input-group-addon" id="discount-delivery-addon" @if( (old('discount_type')??$result['deals_discount_delivery_rules']['discount_type']??$result['deals_promotion_discount_delivery_rules']['discount_type']??false) == "Nominal") style="display: none;" @endif>%</div>
			</div>
		</div>
	</div>
</div>
<div class="form-group" id="discount-delivery-percent-max-div" style="{{ (old('discount_type')??$result['deals_discount_delivery_rules']['discount_type']??$result['deals_promotion_discount_delivery_rules']['discount_type']??false) == "Percent" ? '' : "display: none" }}">
	<div class="row">
		<div class="col-md-3">
			...
			<div class="input-group col-md-12">
				...
				<input type="text" class="form-control text-center digit_mask" name="max_percent_discount" placeholder="" value="{{ old('max_percent_discount')??$result['deals_discount_delivery_rules']['max_percent_discount']??$result['deals_promotion_discount_delivery_rules']['max_percent_discount']??null }}" min="0" oninput="validity.valid||(value='');" autocomplete="off">
			</div>
		</div>
	</div>
</div>
