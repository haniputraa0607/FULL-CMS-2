...
@section('content')
    ...
    <div class="portlet light bordered">
        ...
        <div class="portlet-body form">
            <form action="{{url('role/update/'.$detail['id_role'])}}" role="form" method="POST" class="form-horizontal">
                {{ csrf_field() }}
                <div class="form-body">
                    <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <div class="input-icon right">
                                <input type="text" placeholder="Role name" class="form-control" name="role_name" value="{{ $detail['role_name'] }}" required>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                            ...
                            <div class="input-icon right">
                                <select  class="form-control select2 select2-multiple-job-level" name="id_job_level" data-placeholder="Select job level" required>
                                    ...
                                    @foreach($job_levels as $jobLevel)
                                        <option value="{{$jobLevel['id_job_level']}}" @if($detail['id_job_level'] == $jobLevel['id_job_level']) selected @endif>{{$jobLevel['job_level_name']}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="md-checkbox-inline col-md-9" style="text-align:left">
                                ...
                                @foreach($feature_all as $key2 => $feature)
                                    @if($feature['feature_module'] == $module)
                                        <?php
                                        $stat = false;
                                        foreach($detail['roles_features'] as $roleFeature){
                                            if($feature['id_feature'] == $roleFeature['id_feature']){
                                                $stat = true;
                                                $y++;
                                            }
                                        }
