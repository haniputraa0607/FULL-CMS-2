...
@section('content')
    ...
    <div class="portlet light bordered">
        ...
        <div class="portlet-body form">
            <form action="{{url('product-service/position/assign')}}" method="post">
                {{ csrf_field() }}
                <div class="row">
                    <div class="col-md-6">
                        <div class="dd">
                            <ol class="dd-list">
                                @foreach($product as $p)
                                    <li class="dd-item" data-id="{{$p['id_product']}}">
                                        <div class="dd-handle"><i class="sort-icon fa fa-arrows"></i> {{$p['product_code']}} - {{$p['product_name']}}</div>
                                        <input type="hidden" name="product_ids[]" value="{{$p['id_product']}}">
                                    </li>
                                @endforeach
                            </ol>
                        </div>
